...
export default async function handler(
  ...
  try {
    ...
    const { data, error } = await admin
      .from("contracts")
      .select(
        "id, reference, title, contract_type, description, status, requires_signature, effective_date, expiry_date, currency, contract_value, assignment_id, created_at, updated_at, owner:organisations!contracts_owner_organisation_id_fkey(id, legal_name, trading_name), counterparty:organisations!contracts_counterparty_organisation_id_fkey(id, legal_name, trading_name), contract_parties(id, organisation_id, party_role, signatory_name, signatory_email, signature_required, signing_order), contract_versions(id, version_label, original_filename, size_bytes, malware_scan_status, locked_at, drive_sync_status, drive_source_file_id, drive_final_file_id, drive_certificate_file_id, intermediate_storage_path, intermediate_content_sha256, final_scan_status, certificate_scan_status, final_storage_path, certificate_storage_path, signed_at, created_at, superseded_at, contract_signature_events(id, contract_party_id, signing_order, event_role, organisation_name, signer_name, signer_email, signer_title, signed_at))",
      )
      .neq("status", "archived")
